<script setup>
  ...
  const { data: contractInfo, error, isError, isLoading, isRefetching, refetch } = useGetContract(contract_id);

  watch(contractInfo, async() =>{
    generateChartData();
  })
  const { data: paymentHistory, isLoading: hLoading} = useGetPaymentHistory(contract_id);
  ...
  const loadPaymentHistory = async()=>{
    paymentHistory.value = await Connect.canister(contract_id, 'contract').history();
  }
  const cancelContract = async()=>{
      let _me = await useCancelContract(contract_id);
      ...
      <div class="card mb-6 mb-xl-9">
        <div class="card-body pt-9 pb-0">
          ...
          <ul class="nav nav-stretch nav-line-tabs nav-line-tabs-2x border-transparent fs-5 fw-bold">
            ...
            <li class="nav-item">
              <a class="nav-link text-active-primary py-5 me-6 " href="#">Histories</a>
            </li>
            <li class="nav-item">
                      ...
                      <td>
                        ...
                        <div class="d-flex align-items-center">
                          ...
                          <div class="d-flex flex-column justify-content-center">
                            ...
                            <div class="fw-semibold text-gray-500">{{ moment.unix(Number(payment[1].time)).format('lll') }}</div>
                          </div>
                          <!--end::Info-->
                        </div>
                        <!--end::User-->
                      </td>
                      <td>{{ Number(payment[1].amount)/config.E8S }}</td>
                      <td> 30 </td>
                      <td> 10 </td>
                    </tr>
                  </tbody>
